                    ...
                    for j in range(1, (hostsPerSwitch + 1)):
                         if differentSubnets == 0:
                              host = self.addHost('h%s%s' % (r, hostsPerSwitch * i + j),
                                                  Mask='255.255.0.0')  # ,ip="10.0.0.%s/24" %((r-1)*k*hostsPerSwitch + hostsPerSwitch*i+j)) # % (r,(hostsPerSwitch*i+j)), defaultRoute = "via 10.0.%s.254" % (r) )
                         else:
                              host = self.addHost('h%s%s' % (r, hostsPerSwitch * i + j),
                                                  ip="10.0.%s.%s/24" % ((r - 1), hostsPerSwitch * i + j),
                                                  defaultRoute="via 10.0.%s.254" % r)  # % (r,(hostsPerSwitch*i+j)), defaultRoute = "via 10.0.%s.254" % (r) )
...
class DataTraffic:
     ...
     def readCoeff(self):
        ...
        coeff = self.readLastLine(coeffile)
